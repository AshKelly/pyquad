...
integration_methods = ["qags", "cquad"]
...
def test_grid_single_column():
    for integration_method in integration_methods:
        ...
        res2 = pyquad.quad_grid(function1, 0, 1, grid, (1.0, 1.0),
                                integration_method=integration_method)[0]
...
def test_grid_one_column_with_args():
    for integration_method in integration_methods:
        ...
        res2 = pyquad.quad_grid(function2, 0, 1, grid, args=(1.0, 2.0),
                                integration_method=integration_method)[0]
...
def test_grid_two_column_with_args():
    for integration_method in integration_methods:
        ...
        res2 = pyquad.quad_grid(function2, 0, 1, grid, args=(1.0),
                                integration_method=integration_method)[0]
...
def test_grid_full_column():
    for integration_method in integration_methods:
        ...
        res2 = pyquad.quad_grid(function1, 0, 1, grid, integration_method=integration_method)[0]

        assert np.abs(np.sum(res) - np.sum(res2)) < 1e-5

test_grid_full_column()
