...
gsl_integration_cquad_workspace *gsl_integration_cquad_workspace_alloc(
    ...
    w = (gsl_integration_cquad_workspace *)malloc(
        sizeof(gsl_integration_cquad_workspace));

    /* Allocate the intervals */
    if ((w->ivals = (gsl_integration_cquad_ival *)malloc(
             sizeof(gsl_integration_cquad_ival) * n)) == NULL) {
        free(w);
    }
...
void gsl_integration_cquad_workspace_free(gsl_integration_cquad_workspace *w) {
    ...
    if (w->ivals != NULL) free(w->ivals);

    /* Free the heap */
    if (w->heap != NULL) free(w->heap);
...
static void Vinvfx(const double *fx, double *c, const int d) {
    ...
    switch (d) {
        case 0:
            for (i = 0; i <= 4; i++) {
                c[i] = 0.0;
                for (j = 0; j <= 4; j++) c[i] += V1inv[i * 5 + j] * fx[j * 8];
            }
            break;
        case 1:
            for (i = 0; i <= 8; i++) {
                c[i] = 0.0;
                for (j = 0; j <= 8; j++) c[i] += V2inv[i * 9 + j] * fx[j * 4];
            }
            break;
        case 2:
            for (i = 0; i <= 16; i++) {
                c[i] = 0.0;
                for (j = 0; j <= 16; j++) c[i] += V3inv[i * 17 + j] * fx[j * 2];
            }
            break;
        case 3:
            for (i = 0; i <= 32; i++) {
                c[i] = 0.0;
                for (j = 0; j <= 32; j++) c[i] += V4inv[i * 33 + j] * fx[j];
            }
            break;
    }
}
...
static void downdate(double *c, int n, int d, int *nans, int nnans) {
    ...
    for (i = 0; i <= n + 1; i++) b_new[i] = bee[bidx[d] + i];
    for (i = 0; i < nnans; i++) {
        b_new[n + 1] = b_new[n + 1] / Lalpha[n];
        b_new[n] = (b_new[n] + xi[nans[i]] * b_new[n + 1]) / Lalpha[n - 1];
        for (j = n - 1; j > 0; j--)
            b_new[j] = (b_new[j] + xi[nans[i]] * b_new[j + 1] -
                        Lgamma[j + 1] * b_new[j + 2]) /
                       Lalpha[j - 1];
        for (j = 0; j <= n; j++) b_new[j] = b_new[j + 1];
        alpha = c[n] / b_new[n];
        for (j = 0; j < n; j++) c[j] -= alpha * b_new[j];
        c[n] = 0;
        n--;
    }
}
...
int gsl_integration_cquad(const gsl_function *f, double a, double b,
                          ...
                          void *void_ws, double *result, double *abserr) {
    ...
    gsl_integration_cquad_workspace *ws =
        (gsl_integration_cquad_workspace *)void_ws;
    ...
    if (epsabs < 0.0 || epsrel < 0.0) return GSL_EBADTOL;
    if (epsabs <= 0 && epsrel < GSL_DBL_EPSILON) return GSL_EBADTOL;
    ...
    for (i = 0; i < nnans; i++) iv->fx[nans[i]] = GSL_NAN;
    iv->a = a;
    iv->b = b;
    iv->depth = 3;
    iv->rdepth = 1;
    ...
    nc = sqrt(nc);
    iv->err = ncdiff * 2 * h;
    if (ncdiff / nc > 0.1 && iv->err < 2 * h * nc) iv->err = 2 * h * nc;

    /* Initialize the heaps. */
    for (i = 0; i < ws->size; i++) ws->heap[i] = i;
    ...
    while (nivals > 0 && err > 0.0 &&
        ...
        /*      printf
                ("cquad: processing ival %i (of %i) with [%e,%e] int=%e, err=%e,
        ...
        */
        ...
        if ((m + h * xi[0]) >= (m + h * xi[1]) ||
            ...
            /*          printf
                        ("cquad: dumping ival %i (of %i) with [%e,%e] int=%e,
               err=%e, depth=%i\n", ws->heap[0], nivals, iv->a, iv->b,
               iv->igral, iv->err, iv->depth);
            */
            ...
            while (2 * i + 1 < nivals) {
                ...
                /* If the j+1st entry exists and is larger than the jth,
                   use it instead. */
                if (j + 1 < nivals && ws->ivals[ws->heap[j + 1]].err >=
        ...
        /* Do we need to split this interval? */
            ...
            if (nnans > 0) {
                ...
                for (i = 0; i < nnans; i++) ivl->fx[nans[i]] = GSL_NAN;
            }
            for (i = 0; i <= n[d]; i++) {
                ivl->c[idx[d] + i] = 0.0;
                for (j = i; j <= n[d]; j++)
            ...
            ivl->ndiv =
                iv->ndiv + (fabs(iv->c[0]) > 0 && ivl->c[0] / iv->c[0] > 2);
            ...
            if (nnans > 0) {
                ...
                for (i = 0; i < nnans; i++) ivr->fx[nans[i]] = GSL_NAN;
            }
            for (i = 0; i <= n[d]; i++) {
                ivr->c[idx[d] + i] = 0.0;
                for (j = i; j <= n[d]; j++)
                    ivr->c[idx[d] + i] +=
                        Tright[i * 33 + j] * iv->c[idx[d] + j];
            }
            ncdiff = 0.0;
            for (i = 0; i <= n[0]; i++) {
            ...
            /* Check for divergence. */
            ivr->ndiv =
                iv->ndiv + (fabs(iv->c[0]) > 0 && ivr->c[0] / iv->c[0] > 2);
            ...
            while (2 * i + 1 < nivals - 1) {
                j = 2 * i + 1;
                if (j + 1 < nivals - 1 && ws->ivals[ws->heap[j + 1]].err >=
                                              ws->ivals[ws->heap[j]].err)
                    j++;
                if (ws->ivals[ws->heap[j]].err <= ws->ivals[ws->heap[i]].err)
                    break;
    ...
    /*  for (i = 0; i < nivals; i++)
        {
          ...
          printf
            ("cquad: ival %i (%i) with [%e,%e], int=%e, err=%e, depth=%i,
